...
/// The browser forms render their hidden inputs in sorted key order
/// (`LogisticsForm::new`). A browser POST is exactly these pairs,
/// urlencoded, in rendered order — so POST them server-to-server verbatim
/// and stage must accept the order (the signed `1|` reply, RtnCode=300).
/// This pins FIELD-ORDER acceptance specifically; browser encoding
/// equivalence beyond that (charset, multipart) is out of scope. Creates
/// one stage CVS order, like the create tests do.
/// requests-style form encoding (quote_plus): alnum + '_.-~' literal,
/// space -> '+', everything else uppercase %XX — what a browser sends for
...
/// POST the pairs IN THE GIVEN ORDER (the browser's exact field sequence
/// for a rendered form), urlencoded.
async fn form_post_ordered(endpoint: &str, pairs: &[(String, String)]) -> String {
    let body = pairs
        .iter()
...
async fn browser_form_field_order_is_accepted_by_stage() {
    let client = sdk();
    let form = client
        .logistics_create_form(&LogisticsCreateInput {
            ...
            // Deliberately NO ClientReplyURL: with one, Express/Create is a
            // browser endpoint answering an HTML flow page; without it the
            // same signed fields answer the API-shaped `1|<signed query>`
            // reply — the crisp acceptance signal for the field order.
            ..Default::default()
        })
        .expect("form builds");
    // Defense in depth: the rendered pairs must themselves be sorted (also
    // pinned offline in tests/logistics_wire.rs).
    let pairs = form.pairs();
    assert!(
        pairs.windows(2).all(|w| w[0].0 < w[1].0),
        "rendered pairs must be sorted: {pairs:?}"
    );
    let text = form_post_ordered(form.action(), pairs).await;
    println!("reply = {}", &text[..text.len().min(300)]);
    assert!(text.starts_with("1|"), "accepted: {text}");
    assert!(
        text.contains("RtnCode=300") || text.contains("RtnCode%3d300"),
        "a fresh CVS order answers RtnCode=300: {text}"
    );
}
